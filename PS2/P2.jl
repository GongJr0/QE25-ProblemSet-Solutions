...
df50 = DataFrame(
    Parameter = ["ρ", "p"],
    Estimate = θ̂,
    Std_Dev = [std_ρ̂50, std_p̂50],
    Bias = [bias_ρ̂50, bias_p̂50]
)
pretty_table(df50)

std_ρ̂200 = std(θ_est200[:, 1])
std_p̂200 = std(θ_est200[:, 2])
bias_ρ̂200 = mean(θ_est200[:, 1]) - 0.9
bias_p̂200 = mean(θ_est200[:, 2]) - 0.8
df200 = DataFrame(
    Parameter = ["ρ", "p"],
    Estimate = θ̂,
    Std_Dev = [std_ρ̂200, std_p̂200],
    Bias = [bias_ρ̂200, bias_p̂200]
)
pretty_table(df200)
