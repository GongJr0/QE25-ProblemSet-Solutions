...
# P(DEATH | j)
π(J::Int64, j::Int) = j==J ? 1 : min(5e-4 * 1.14^j, 1)
...
function compute_amax(J::Int64, ȳ::Float64=1.0)
    a = 0.0
    for j in 1:J
        a = (1+r)*a + y(j, ȳ)
    end
...
V, cpol, apol, agrid = solve_model(60, 1.0, 0.5)


println("Is V defined for all j: ", all(isfinite, V[60, :]))
println("Are all consumption values non-negative: ", all(cpol .>= 0))
println("Are all asset policy values non-negative: ", all(apol .>= 0))
println("Are al asset policy values within bounds: ", all(apol[j, i] .<= (1+r)*agrid[i] + y(j, 1.0) for j in 1:60, i in 1:500))
